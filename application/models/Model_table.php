...
class Model_Table extends CI_Model{
    ...
    public function customerDue($clauses = "") {
        ...
        $dueResult = $this->db->query("
            ...
            (select ifnull(sum(cp.CPayment_amount), 0.00) 
                from tbl_customer_payment cp 
                where cp.CPayment_customerID = c.Customer_SlNo 
                and cp.CPayment_TransactionType = 'CD'
                and cp.CPayment_status = 'a') as discountAmount,
            ...
            (select (billAmount + paidOutAmount) - (paidAmount + returnedAmount + discountAmount)) as dueAmount
            
            from tbl_customer c
            where c.Customer_brunchid = ? $clauses
        ", $branchId)->result();

        return $dueResult;
    }

    public function productStock($productId, $branchId) {
    ...
    public function supplierDue($clauses = '')
    {
        $branchId = $this->session->userdata('BRANCHid');
        ...
        $supplierDues = $this->db->query("
            ...
            (select ifnull(sum(sp2.SPayment_amount), 0.00) from tbl_supplier_payment sp2 
                where sp2.SPayment_customerID = s.Supplier_SlNo 
                and sp2.SPayment_TransactionType = 'CD'
                and sp2.SPayment_status = 'a') as discountAmount,
            ...
            (select (bill + cashReceived) - (paid + returned + discountAmount)) as due

            from tbl_supplier s
            where s.Supplier_brinchid = '$branchId' 
            and s.Status = 'a' and s.Supplier_Type != 'G' $clauses
        ")->result();
