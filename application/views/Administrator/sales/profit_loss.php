		...
		<div class="col-md-12">
			<form class="form-inline" v-on:submit.prevent="getProfitLoss">
				<div class="form-group" style="margin-right: 15px;">
					...
					<v-select class="notranslate" v-bind:options="customers" v-model="selectedCustomer" label="display_name" placeholder="Select Customer"></v-select>
				</div>

				<div class="form-group">
					<label>Date from </label>
					<input type="date" class="form-control notranslate" v-model="filter.dateFrom">
				</div>

				<div class="form-group">
					<label>to </label>
					<input type="date" class="form-control notranslate" v-model="filter.dateTo">
				</div>
			...
			<div class="table-responsive" id="reportTable">
				<table id="table1">
					...
					<tfoot style="display:none;font-weight:bold;background-color:#e9dcdc;" v-bind:style="{display: reportData.length > 0 ? '' : 'none'}">
						...
						<tr>
							<td colspan="4" style="text-align:right;">Total Supplier Commission (+)</td>
							<td colspan="2"></td>
							<td style="text-align:right;">{{ otherIncomeExpense.supplier_commission | decimal }}</td>
						</tr>

						<tr>
							<td colspan="4" style="text-align:right;">Total Sales Discount (-)</td>
							<td colspan="2"></td>
						...
						<tr>
							<td colspan="4" style="text-align:right;">Total Customer Discount (-)</td>
							<td colspan="2"></td>
							<td style="text-align:right;">{{ otherIncomeExpense.customer_discount | decimal }}</td>
						</tr>
						...
						<tr>
							...
							<td colspan="2"></td>
							<td style="text-align:right;">
								{{  ((parseFloat(totalProfit) + parseFloat(otherIncomeExpense.income) + parseFloat(otherIncomeExpense.supplier_commission)) - 
									(parseFloat(totalDiscount) + parseFloat(otherIncomeExpense.returned_amount) + parseFloat(otherIncomeExpense.damaged_amount) + parseFloat(otherIncomeExpense.expense) + parseFloat(otherIncomeExpense.employee_payment) + parseFloat(otherIncomeExpense.customer_discount))).toFixed(2) }}
							</td>
						</tr>
					</tfoot>
				</table>
