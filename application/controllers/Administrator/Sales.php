    ...
    public function addSales()
    {
        ...
        try {
            ...
            $sales = array(
                ...
                'SaleMaster_LabourCost' => $data->sales->labourCost,
        ...
        try {
            ...
            foreach ($data->cart as $product) {
                $returnDetails = array(
                    ...
                    'AddBy' => $this->session->userdata("FullName"),
                    'AddTime' => date('Y-m-d H:i:s'),
                    'SaleReturnDetails_brunchID' => $product->SaleDetails_BranchId,
                );
                ...
                $this->db->query("
                    ...
                    where product_id = ?
                    and branch_id = ?
                ", [$product->return_quantity, $product->Product_IDNo, $product->SaleDetails_BranchId]);
            }
        ...
        try {
            ...
            foreach ($oldDetails as $product) {
                $this->db->query("
                    ...
                    where product_id = ?
                    and branch_id = ?
                ", [$product->SaleReturnDetails_ReturnQuantity, $product->SaleReturnDetailsProduct_SlNo, $product->SaleReturnDetails_brunchID]);
            }
...
<?php
    ...
    public function  deleteSales()
    {
        ...
        try {
            ...
            $saleDetails = $this->db->select('Product_IDNo, SaleDetails_TotalQuantity,SaleDetails_BranchId')->where('SaleMaster_IDNo', $saleId)->get('tbl_saledetails')->result();

            foreach ($saleDetails as $detail) {
                /*Get Product Current Quantity*/
                $totalQty = $this->db->where(['product_id' => $detail->Product_IDNo, 'branch_id' => $detail->SaleDetails_BranchId])->get('tbl_currentinventory')->row()->sales_quantity;

                /* Subtract Product Quantity form  Current Quantity  */
                $newQty = $totalQty - $detail->SaleDetails_TotalQuantity;

                /*Update Sales Inventory*/
                $this->db->set('sales_quantity', $newQty)->where(['product_id' => $detail->Product_IDNo, 'branch_id' => $detail->SaleDetails_BranchId])->update('tbl_currentinventory');
            }
